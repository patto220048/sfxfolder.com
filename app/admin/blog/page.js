...
export default function BlogAdminPage() {
  ...
  const handleAIGenerateText = async () => {
    ...
    try {
      ...
      // Pre-fill image prompt dynamically generated by AI to match the article perfectly!
  ...
  // Upload local image directly to Supabase storage bucket site-assets
  const handleLocalImageUpload = async (e) => {
    ...
    try {
      ...
      formDataToSend.append("slug", formData.slug || `sfx-blog-${Date.now()}`);

      console.log("[Local Upload] Sending local file to API upload endpoint...");
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Gặp lỗi khi upload ảnh");
      ...
      setSuccess("Đã upload ảnh bìa lên hệ thống thành công!");
      setTimeout(() => setSuccess(""), 4000);
    } catch (err) {
      setError(err.message);
    } finally {
      setUploadingFile(false);
      // Clear input so upload can be triggered again with same file if needed
      e.target.value = "";
    }
  };
                    ...
                    <div className={styles.cardGlass}>
                      ...
                      <div className={styles.inputGroup}>
                        ...
                        <div style={{ display: "flex", gap: "8px" }}>
                          ...
                          >
                            {uploadingFile ? (
                              <Loader2 size={16} className={`${styles.spinner} animate-spin`} />
                            ) : (
                              <Upload size={16} />
                            )}
                            <input
